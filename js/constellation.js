...
drawProperty(ctx,w,h,t){ const ax=$('#axis-x').value, ay=$('#axis-y').value;
  ...
  const pad=64,plotTop=w<700?205:215;
  ctx.strokeStyle='rgba(244,240,232,.12)'; ctx.beginPath();
  ctx.moveTo(pad,h-pad); ctx.lineTo(w-30,h-pad); ctx.moveTo(pad,h-pad); ctx.lineTo(pad,plotTop); ctx.stroke();
  ...
  ctx.save(); ctx.translate(26,(plotTop+h-pad)/2); ctx.rotate(-Math.PI/2);
  ...
  MAT_LIST.forEach(id=>{ const m=MATERIALS[id]; const vx=val(m,ax),vy=val(m,ay); if(vx<=0&&vy<=0) return;
    ...
    const px=pad+ (tf(vx,ax)-x0)/(x1-x0||1)*(w-pad-50), py=(h-pad)-(tf(vy,ay)-y0)/(y1-y0||1)*(h-pad-plotTop);
